...
11. **VieNeu ngắt nhịp sai ở câu nhập nhằng cú pháp** ("Thầy giáo mới | ngay từ sáng" đọc thành "Thầy giáo | mới ngay từ sáng") và **lặp cụm từ** ở câu vốn đã có từ lặp ("đi đi, đi đi" đọc ba lần). Không tự phát hiện được, chỉ bắt bằng tai qua QA. Ngắt nhịp: thêm dấu phẩy vào bản đọc (`sua_cach_doc.csv`). Lặp: đã thử `repetition_penalty` 1,35/1,5 và `temperature` 0,5/0,3 — không nhất quán. Đọc lại N lần rồi chọn bản tốt nhất; **chấm điểm bằng ASR** (faster-whisper small, int8, CPU): nghe ngược từng bản rồi so với văn bản gốc. Ban đầu chấm bằng thời lượng so với kỳ vọng, nhưng cách đó **không phát hiện được lặp ở câu mà văn bản vốn có từ lặp** ("đi đi, đi đi" đọc ba lần chỉ dài thêm 0,3 s, nằm trong sai số) — người nghe phát hiện ra, ASR mới phân biệt được. Xem `scripts/06_doc_lai.py`.
...
13. **Dò ngắt nhịp sai bằng mốc thời gian của ASR: thất bại.** Ý tưởng là tìm khoảng nghỉ dài ở chỗ không có dấu câu. Nghe kiểm ba chỗ bị báo nặng nhất thì hỏng cả ba: một chỗ báo "nghỉ 1,16 giây" nhưng nghe không có nghỉ nào (mốc thời gian Whisper là ước lượng theo attention, lệch mạnh quanh tên riêng và số), hai chỗ còn lại nghỉ đúng ngữ pháp vì tiếng Việt ngắt được ở chỗ không có dấu câu. Muốn làm cần forced alignment thay vì ASR, và luật ngắt nhịp tiếng Việt. Giữ lại hàm trong `asr_kiem.ngat_nhip` nhưng không dùng để báo lỗi.
